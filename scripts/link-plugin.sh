...
[[ -f $root/manifest.json ]] || die "Not an omatide checkout: missing $root/manifest.json"
...
unlink() {
  ...
  if [[ -d $backup ]]; then
    ...
    printf 'Unlinked %s. No clone to restore; omarchy plugin add https://github.com/shieldsworks/omatide.git --enable\n' "$target"
  fi
  rescan
}
