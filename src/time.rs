...
/// The UTC calendar year containing `t`. Node factors are held for a
/// whole year at a time, so this decides which set a moment uses.
pub fn year_of(t: i64) -> i64 {
    civil_from_days(t.div_euclid(86_400)).0
}

/// Noon on 2 July, the middle of a year, where NOAA computes the node
/// factors it then holds for the whole of it.
pub fn mid_year(year: i64) -> i64 {
    unix(year, 7, 2, 12, 0, 0)
}
...
mod tests {
    ...
    #[test]
    fn finds_the_year_and_its_middle() {
        assert_eq!(year_of(unix(2026, 1, 1, 0, 0, 0)), 2026);
        assert_eq!(year_of(unix(2026, 12, 31, 23, 59, 59)), 2026);
        assert_eq!(year_of(unix(2025, 12, 31, 23, 59, 59)), 2025);
        assert_eq!(mid_year(2026), unix(2026, 7, 2, 12, 0, 0));
        assert_eq!(year_of(mid_year(2031)), 2031);
    }
